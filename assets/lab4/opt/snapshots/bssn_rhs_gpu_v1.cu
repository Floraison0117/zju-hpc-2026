...
__global__ __launch_bounds__(256, 2) void rhs_deriv_kernel(
    ...
    double* betaxx_out, double* betaxy_out, double* betaxz_out,
    double* betayx_out, double* betayy_out, double* betayz_out,
    double* betazx_out, double* betazy_out, double* betazz_out,
    double* chix_out, double* chiy_out, double* chiz_out,
    double* gupxx_out, double* gupxy_out, double* gupxz_out,
    double* gupyy_out, double* gupyz_out, double* gupzz_out,
...
__global__ __launch_bounds__(256, 2) void rhs_core_kernel(
    int ex0, int ex1, int ex2, double T, double* X, double* Y, double* Z,
    double* chi, double* trK,
    double* dxx, double* gxy, double* gxz,
    double* dyy, double* gyz, double* dzz,
    ...
    double alpn1 = Lap[idx] + ONE;
    double chin1 = chi[idx] + ONE;
    double val_trK = trK[idx];

    double betaxx = betaxx_out[idx]; double betaxy = betaxy_out[idx]; double betaxz = betaxz_out[idx];
    double betayx = betayx_out[idx]; double betayy = betayy_out[idx]; double betayz = betayz_out[idx];
    double betazx = betazx_out[idx]; double betazy = betazy_out[idx]; double betazz = betazz_out[idx];
    double div_beta = betaxx + betayy + betazz;
    double chix = chix_out[idx]; double chiy = chiy_out[idx]; double chiz = chiz_out[idx];
    double gupxx = gupxx_out[idx]; double gupxy = gupxy_out[idx]; double gupxz = gupxz_out[idx];
    double gupyy = gupyy_out[idx]; double gupyz = gupyz_out[idx]; double gupzz = gupzz_out[idx];
...
void gpu_compute_rhs_bssn_launch( // launch kernel with device pointers
    ...
    // static scratch buffer for the split kernels' 18 intermediate arrays
    int scratch_size = ex[0] * ex[1] * ex[2];
    double* d_scratch = nullptr;
    cudaMallocAsync(&d_scratch, (size_t)18 * scratch_size * sizeof(double), stream);
    double* d_betaxx_out = d_scratch + 0 * scratch_size;
    double* d_betaxy_out = d_scratch + 1 * scratch_size;
    double* d_betaxz_out = d_scratch + 2 * scratch_size;
    double* d_betayx_out = d_scratch + 3 * scratch_size;
    double* d_betayy_out = d_scratch + 4 * scratch_size;
    double* d_betayz_out = d_scratch + 5 * scratch_size;
    double* d_betazx_out = d_scratch + 6 * scratch_size;
    double* d_betazy_out = d_scratch + 7 * scratch_size;
    double* d_betazz_out = d_scratch + 8 * scratch_size;
    double* d_chix_out = d_scratch + 9 * scratch_size;
    double* d_chiy_out = d_scratch + 10 * scratch_size;
    double* d_chiz_out = d_scratch + 11 * scratch_size;
    double* d_gupxx_out = d_scratch + 12 * scratch_size;
    double* d_gupxy_out = d_scratch + 13 * scratch_size;
    double* d_gupxz_out = d_scratch + 14 * scratch_size;
    double* d_gupyy_out = d_scratch + 15 * scratch_size;
    double* d_gupyz_out = d_scratch + 16 * scratch_size;
    double* d_gupzz_out = d_scratch + 17 * scratch_size;

    // 1. Kernel 1: Derivatives & Connection Coefficients
    rhs_deriv_kernel<<<grid, block, 0, stream>>>(
        ...
        d_betaxx_out, d_betaxy_out, d_betaxz_out,
        d_betayx_out, d_betayy_out, d_betayz_out,
        d_betazx_out, d_betazy_out, d_betazz_out,
        d_chix_out, d_chiy_out, d_chiz_out,
        d_gupxx_out, d_gupxy_out, d_gupxz_out,
        d_gupyy_out, d_gupyz_out, d_gupzz_out,
        symmetry, lev, eps, co
    );
    // 2. Kernel 2: RHS core
    rhs_core_kernel<<<grid, block, 0, stream>>>(
        ex[0], ex[1], ex[2], T, d_X, d_Y, d_Z,
        d_chi, d_trK,
        d_dxx, d_gxy, d_gxz,
        d_dyy, d_gyz, d_dzz,
