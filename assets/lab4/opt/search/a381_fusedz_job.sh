#!/bin/bash
# Iter38 A38-1 fused-z L0+L1: build candidate, ptxas check, 2-step A/B.
set -uo pipefail
...
EV=$CAND/evidence/a381
mkdir -p "$EV"; exec > "$EV/job.log" 2>&1
echo "=== START A381 $CAND $(date -u) ==="
...
./compile.sh -DCMAKE_CUDA_FLAGS="-Xptxas -v" > "$EV/build-a381.log" 2>&1 \
  && echo BUILD_OK || { echo BUILD_FAIL; grep -iE "error" "$EV/build-a381.log" | head -20; exit 3; }
echo "=== global_interp ptxas (candidate A38-1) ==="
for K in global_interp_multi_kernel global_interp_amr_kernel; do
  echo "-- $K"; grep -A8 "Function.*$K" "$EV/build-a381.log" | grep -iE "registers|spill|stack" | head -3
done
...
run_v cand1 "$CAND/build-a381"
run_v cand2 "$CAND/build-a381"
