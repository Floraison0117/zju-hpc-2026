#!/bin/bash
# P-R3BATCH: restrict3 variable-batched launch (launch-structure lever).
#   Mechanism: restrict3 (294,975 launches, 24.58s/3.98%, avg 0.083ms,
#   blocks/launch 1/29/72, waves 1.04) is launched once per (block-pair,
#   variable).  Batch all vars of one block pair into one 2-D-grid launch
#   (grid.y = var_idx) -> launch count /24, geometry computed once on host.
#   Bit-exact by construction (identical per-thread arithmetic on disjoint
#   dst slots).  NOTE: differs from RK4 batch (which was dead, F=0.9896):
#   restrict3 has 1.04 waves (tiny grids -> SM idle), RK4 had 125-945 blocks.
#   Level-0: build -Xptxas -v, grep restrict3_multi_kernel.
#   Level-1: 2-step interleaved A/B, analysis OFF.
#   base = deployed 26bcd binary.
set -uo pipefail
ROOT=$(ls -dt ~/lab4-gpu-cand-r3batch-* 2>/dev/null | head -1)
...
ulimit -s unlimited
EV="$ROOT/evidence/r3batch"
mkdir -p "$EV"; exec > "$EV/job.log" 2>&1
echo "=== START R3BATCH $(date -u) ROOT=$ROOT ==="
...
export AMSS_BUILD_DIR="$ROOT/build-r3batch"
rm -rf "$AMSS_BUILD_DIR"
./compile.sh -DCMAKE_CUDA_FLAGS="-Xptxas -v" > "$EV/build-r3batch.log" 2>&1 \
  && echo "BUILD_R3BATCH_OK" || { echo "BUILD_R3BATCH_FAIL"; grep -iE "error" "$EV/build-r3batch.log" | head -20; exit 3; }
echo "=== multi kernel ptxas (candidate) ==="
grep -A4 "Function.*restrict3_multi_kernel" "$EV/build-r3batch.log" | grep -iE "registers|spill|stack" | head -4
...
run_v cand1 "$ROOT/build-r3batch"
run_v cand2 "$ROOT/build-r3batch"
