#!/bin/bash
# Iter33 (P33) Level-2: 100-step full run + check.sh on the prolong3 group
# multi-output candidate (2x2x2 fine group shares one 6x6x6 coarse cube).
# Base for comparison: deployed 26bcd OJ-sim 605.78s (real OJ 601.364s).
set -uo pipefail
...
ulimit -s unlimited
EV="$ROOT/evidence/p33-l2"
mkdir -p "$EV"; exec > "$EV/job.log" 2>&1
echo "=== START P33_L2 $(date -u) ROOT=$ROOT ==="
...
export AMSS_BUILD_DIR="$ROOT/build-p33-clean"
