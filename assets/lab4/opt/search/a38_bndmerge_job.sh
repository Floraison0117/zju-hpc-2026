#!/bin/bash
# Iter38 A38-BND L0+L1: build merged-boundary candidate, ptxas check, 2-step A/B.
#   Base binary: deployed P313233 build (~/lab4-gpu/build-reprofile/ABEGPU).
#   Candidate: ~/lab4-gpu-cand-a38-bndmerge-<ts> (3 files patched).
#   Gate: bit-exact 8/8 .dat + F>1.0.
set -uo pipefail
CAND=$(cut -d= -f2 ~/cand-a38-path.txt)
...
EV=$CAND/evidence/a38
mkdir -p "$EV"; exec > "$EV/job.log" 2>&1
echo "=== START A38 $CAND $(date -u) ==="
...
cd "$CAND"
export AMSS_BUILD_DIR="$CAND/build-a38"
rm -rf "$AMSS_BUILD_DIR"
./compile.sh -DCMAKE_CUDA_FLAGS="-Xptxas -v" > "$EV/build-a38.log" 2>&1 \
  && echo BUILD_OK || { echo BUILD_FAIL; grep -iE "error" "$EV/build-a38.log" | head -20; exit 3; }
echo "=== rhs kernel ptxas (candidate A38) ==="
for K in rhs_kernel_facepure rhs_kernel_facez rhs_kernel; do
  echo "-- $K"; grep -A8 "Function.*$K" "$EV/build-a38.log" | grep -iE "registers|spill|stack" | head -3
done
echo "=== rhs kernel ptxas (deployed base, from build-reprofile) ==="
grep -A8 "Function.*rhs_kernel_facepure" "$ROOT/evidence/reprofile-p313233-*/build.log" 2>/dev/null | grep -iE "registers|spill|stack" | head -3 || echo "no base ptxas log (deployed built without -v)"

# --- 2-step config (OJ: Final=100, Analysis=0.1; A/B: Final=2.0) ---
python3 - <<'PY'
import re
...
run_v cand1 "$CAND/build-a38"
run_v cand2 "$CAND/build-a38"
