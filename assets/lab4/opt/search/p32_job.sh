#!/bin/bash
# Iter32 (P32): sommerfeld boundary compact launch domain.
#   Mechanism: host-side face-condition evaluation (bit-identical to the
#   device-side is_sommerfeld_boundary face tests) -> skip launches with no
#   active face; otherwise launch one compact flat 1-D kernel covering only
#   the active face layers (vs full-volume 3-D grid, blocks 343/512/1120).
#   Bit-exact by construction (verbatim per-point body; duplicate enumeration
#   of edge points writes identical bits).
#   Level-0: candidate build -Xptxas -v, grep sommerfeld kernels.
#   Level-1: 2-step interleaved A/B, analysis OFF (sommerfeld runs every
#            substep regardless of analysis). base = deployed 26bcd binary.
#   Level-2: separate 100-step job if gate passes.
set -uo pipefail
ROOT=$(ls -dt ~/lab4-gpu-cand-p32-sommcompact-* 2>/dev/null | head -1)
...
ulimit -s unlimited
EV="$ROOT/evidence/p32"
mkdir -p "$EV"; exec > "$EV/job.log" 2>&1
echo "=== START P32 $(date -u) ROOT=$ROOT ==="
...
export AMSS_BUILD_DIR="$ROOT/build-p32"
rm -rf "$AMSS_BUILD_DIR"
./compile.sh -DCMAKE_CUDA_FLAGS="-Xptxas -v" > "$EV/build-p32.log" 2>&1 \
  && echo "BUILD_P32_OK" || { echo "BUILD_P32_FAIL"; grep -iE "error" "$EV/build-p32.log" | head -20; exit 3; }
echo "=== sommerfeld ptxas (candidate P32) ==="
grep -A4 "Function.*sommerfeld_rout_compact_kernel" "$EV/build-p32.log" | grep -iE "registers|spill|stack" | head -4
grep -A4 "Function.*sommerfeld_rout_kerneliii" "$EV/build-p32.log" | grep -iE "registers|spill|stack" | head -4

# --- 2-step config, analysis OFF (standard A/B) ---
python3 - <<'PY'
import re
...
run_v cand1 "$ROOT/build-p32"
run_v cand2 "$ROOT/build-p32"
