#!/usr/bin/env bash
# iter16 P7 Level-1 — restrict3 column-fused unroll A/B (OFF/ON x2, 2-step) + RMS
# base = formal tree (build/ABEGPU = deployed 778.24s binary), cand = p7 candidate.
set -uo pipefail
BASE=/home/h3240101033/lab4-gpu
CAND=/home/h3240101033/lab4-gpu-cand-p7-20260824-175747
EV="$CAND/evidence/p7-ab"
mkdir -p "$EV"
exec > "$EV/job-ab.log" 2>&1
echo "=== START P7 A/B $(date -u) ==="
