#!/usr/bin/env bash
# iter14 P5 redo — Level-1 A/B only (ptxas probes already in evidence; GATE=PASS).
# Fixes from job 154113: explicit AMSS_BUILD_DIR per tree (container bakes
# AMSS_BUILD_DIR=/workspace/lab4/build), pre-created GW250118 under output roots.
set -uo pipefail
BASE=/home/h3240101033/lab4-gpu
CAND=/home/h3240101033/lab4-gpu-cand-p5-20260824-073425
EV="$CAND/evidence/p5"
mkdir -p "$EV"
exec > "$EV/job-redo.log" 2>&1
echo "=== START P5-REDO $(date -u) ==="
nvidia-smi -L 2>/dev/null | head -2
echo "--- env AMSS ---"
env | grep -i amss || echo "(no AMSS env)"
...
echo "=== BUILD base (restore formal build/) ==="
( cd "$BASE" && rm -rf build && AMSS_BUILD_DIR="$BASE/build" JOBS=8 ./compile.sh -DCMAKE_CUDA_COMPILER=/usr/local/cuda-13.3/bin/nvcc > "$EV/build-base2.log" 2>&1 ) || echo BUILD_FAIL_BASE
ls -lh "$BASE/build/ABEGPU" 2>/dev/null || echo NO_BASE_BINARY

echo "=== BUILD cand ==="
( cd "$CAND" && rm -rf build && AMSS_BUILD_DIR="$CAND/build" JOBS=8 ./compile.sh -DCMAKE_CUDA_COMPILER=/usr/local/cuda-13.3/bin/nvcc > "$EV/build-cand2.log" 2>&1 ) || echo BUILD_FAIL_CAND
ls -lh "$CAND/build/ABEGPU" 2>/dev/null || echo NO_CAND_BINARY

echo "=== PATCH INPUTS Final=2 (backup formal) ==="
cp "$BASE/AMSS_NCKU_Input.py" "$EV/input.base.backup2.py"
...
cp "$EV/input.base.backup2.py" "$BASE/AMSS_NCKU_Input.py"
