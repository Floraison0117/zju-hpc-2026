#!/bin/bash
# Iter32 (P33): sommerfeld boundary compact launch domain.
...
set -uo pipefail
ROOT=$(ls -dt ~/lab4-gpu-cand-p33-prolonggrp-* 2>/dev/null | head -1)
...
echo "=== START P33 $(date -u) ROOT=$ROOT ==="
...
export AMSS_BUILD_DIR="$ROOT/build-p33"
rm -rf "$AMSS_BUILD_DIR"
./compile.sh -DCMAKE_CUDA_FLAGS="-Xptxas -v" > "$EV/build-p33.log" 2>&1 \
  && echo "BUILD_P33_OK" || { echo "BUILD_P33_FAIL"; grep -iE "error" "$EV/build-p33.log" | head -20; exit 3; }
echo "=== sommerfeld ptxas (candidate P33) ==="
grep -A4 "Function.*prolong3_multi_kernel" "$EV/build-p33.log" | grep -iE "registers|spill|stack" | head -4
grep -A4 "Function.*prolong3_multi_kernel_int" "$EV/build-p33.log" | grep -iE "registers|spill|stack" | head -4
...
run_v cand1 "$ROOT/build-p33"
run_v cand2 "$ROOT/build-p33"
