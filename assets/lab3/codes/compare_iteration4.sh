...
/opt/lab3-venv/bin/python run.py \
  ...
  --repetitions 30 \
  --output-format csv \
  2>&1 | tee "$source_dir/iteration3_iter4_${group}.csv"

cp "$source_dir/student/tilelang_fwd.py.iter4_work" student/tilelang_fwd.py
/opt/lab3-venv/bin/python run.py \
  "${case_args[@]}" \
  --warmup 10 \
  --repetitions 30 \
  --output-format csv \
  2>&1 | tee "$source_dir/iteration4_${group}.csv"
