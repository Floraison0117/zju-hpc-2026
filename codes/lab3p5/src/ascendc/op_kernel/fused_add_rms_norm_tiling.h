...
BEGIN_TILING_DATA_DEF(FusedAddRmsNormTilingData)
    ...
    // Rows processed together in one UB chunk by the aligned batch path
    // (computed from the UB budget and H on the host; 0 = per-row path).
    TILING_DATA_FIELD_DEF(int32_t, rowsPerChunk);
END_TILING_DATA_DEF;
