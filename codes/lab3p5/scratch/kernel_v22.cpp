...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
        ...
        if (this->useBatch) {
            ...
            pipe->InitBuffer(sqBuf, chunkFp16 * 2u);          // FP32 chunk squares (cast+square in place)
    ...
    __aicore__ inline void ProcessAlignedBatch() {
        ...
        while (rowBase < myRows) {
            ...
            AscendC::Add(resOutT, inLocal,
                         ...
                         chunkElems);
            AscendC::Cast(sq, resOutT, AscendC::RoundMode::CAST_NONE, chunkElems);
            AscendC::Mul(sq, sq, sq, chunkElems);
            ...
            for (int32_t row = 0; row < n; ++row) {
                ...
                AscendC::LocalTensor<half> rRow = resOutT[static_cast<uint32_t>(row) * static_cast<uint32_t>(alignH)];
                AscendC::LocalTensor<half> yRow = yT[static_cast<uint32_t>(row) * static_cast<uint32_t>(alignH)];
                AscendC::Muls(yRow, rRow, static_cast<half>(rstd), alignH);
                AscendC::Mul(yRow, yRow, wHalf, alignH);
            }
