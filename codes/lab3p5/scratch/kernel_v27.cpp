...
private:
    ...
    __aicore__ inline void ProcessAlignedBatch() {
        ...
        while (rowBase < myRows) {
            ...
            // (ONE UB->UB DataCopy with srcStride=0 runs on the V pipe, in
            // order before the Mul), so the weight multiply is a single
            // chunk-wide instruction instead of n per-row Mul calls.
            {
                AscendC::DataCopyParams wtParams;
                wtParams.blockCount = static_cast<uint16_t>(n);
                wtParams.blockLen = static_cast<uint16_t>(alignH / 8 > 255 ? 255 : alignH / 8);
                wtParams.srcStride = 0;
                wtParams.dstStride = 0;
                if (alignH / 8 <= 255) {
                    AscendC::DataCopy(wTile, weightFp32, wtParams);
                } else {
                    // H > 2040: fall back to per-row weight multiply.
