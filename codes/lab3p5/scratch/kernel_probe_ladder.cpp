...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
        ...
        this->pipe = pipeIn;
        // PROBE LADDER: rowsPerChunk in [-109,-102] -> run only that many
        // InitBuffer calls, then the caller returns (no further Init work).
        if (tiling.rowsPerChunk <= -102 && tiling.rowsPerChunk >= -109) {
            int32_t nBufs = -(tiling.rowsPerChunk) - 101;   // -102 -> 1, -109 -> 8
            // NOTE: fields like tileElems are not set yet this early in Init;
            // use fixed representative sizes for the probe allocations.
            uint32_t tileBytesFp16 = 2048;
            uint32_t tileBytesFp32 = 4096;
            (void)tileBytesFp16; (void)tileBytesFp32;
            if (nBufs >= 1) pipe->InitBuffer(inBuf, 4096);
            if (nBufs >= 2) pipe->InitBuffer(resOutBuf, 4096);
            if (nBufs >= 3) pipe->InitBuffer(yBuf, 4096);
            if (nBufs >= 4) pipe->InitBuffer(rFp32Buf, 8192);
            if (nBufs >= 5) pipe->InitBuffer(sqBuf, 8192);
            if (nBufs >= 6) pipe->InitBuffer(sumSqBuf, 256);
            if (nBufs >= 7) pipe->InitBuffer(weightHalfBuf, tileBytesFp16);
            if (nBufs >= 8) pipe->InitBuffer(weightFp32Buf, tileBytesFp32);
            return;
        }
...
extern "C" __global__ __aicore__ void fused_add_rms_norm(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
    ...
    const int32_t probe = tilingData.rowsPerChunk;
    if (probe == -100) return;             // tiling read only
    AscendC::TPipe pipe;
    if (probe == -101) return;             // + TPipe ctor
    KernelFusedAddRmsNorm op;
    if (probe <= -102 && probe >= -109) {  // + N InitBuffer calls
