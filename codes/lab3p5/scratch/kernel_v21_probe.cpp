...
extern "C" __global__ __aicore__ void fused_add_rms_norm(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
    ...
    // Probe codes (rowsPerChunk field): 98 pure-return floor; 99 one scalar GM
    // read; 5 direct scalar reads of the whole tiling struct; 6 framework
    // GET_TILING_DATA. Any other value runs the real kernel.
    const __gm__ int32_t* tg = reinterpret_cast<const __gm__ int32_t*>(tiling);
    int32_t code = tg[5];
    if (code == 98) return;
    if (code == 99) { volatile int32_t v = tg[0]; (void)v; return; }
    if (code == 5) {
        // Direct scalar reads of the tiling struct fields (no MTE2, no UB).
