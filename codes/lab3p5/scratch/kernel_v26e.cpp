...
private:
    ...
    __aicore__ inline void ProcessAlignedBatch() {
        ...
        while (rowBase < myRows) {
            ...
            if ((alignH & 511) == 0) {
                // BlockReduceSum tree: BS1 (in <=4096-element slices, the
                // empirically safe counter-mask span) 8->1 per 32B block,
                // BS2 again, then one WRS per row (perRow = alignH/64 partials
                // per row, block-aligned because alignH is a multiple of 512).
                const int32_t perRow = alignH / 64;
                AscendC::SetMaskCount();
                int32_t off = 0;
                const int32_t total = n * alignH;
                while (off < total) {
                    int32_t len = total - off;
                    if (len > 4096) len = 4096;
                    AscendC::SetVectorMask<float, AscendC::MaskMode::COUNTER>(len);
                    AscendC::BlockReduceSum<float, false>(sq2[off / 8], sq[off], len / 64,
                                                          AscendC::MASK_PLACEHOLDER, 1, 1, 8);
                    off += len;
                }
                AscendC::SetVectorMask<float, AscendC::MaskMode::COUNTER>(total / 8);
                AscendC::BlockReduceSum<float, false>(sumTmp, sq2, total / 512,
                                                      AscendC::MASK_PLACEHOLDER, 1, 1, 8);
                AscendC::SetVectorMask<float, AscendC::MaskMode::COUNTER>(perRow);
                for (int32_t row = 0; row < n; ++row) {
                    AscendC::WholeReduceSum<float, false>(sumSqArr[row], sumTmp[row * perRow],
                                                         perRow, 1, 1, 1, perRow / 8);
                }
                AscendC::SetMaskNorm();
                AscendC::ResetMask();
            } else {
