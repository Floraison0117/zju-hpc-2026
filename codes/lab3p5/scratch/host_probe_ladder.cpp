...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
    ...
    // PROBE LADDER: env override rowsPerChunk with a probe code.
    // Negative: -100 tiling-only / -101 +TPipe / -102..-109 +N InitBuffers.
    // Positive: 4/5/6 sync-probe selectors, 99 branch-free baseline.
    if (const char* probeEnv = getenv("LAB35_PROBE")) {
        int probeVal = atoi(probeEnv);
        if ((probeVal <= -100 && probeVal >= -109) ||
            probeVal == 4 || probeVal == 105 || probeVal == 106 ||
            probeVal == 7 || probeVal == 8 ||
            probeVal == 96 || probeVal == 97 || probeVal == 98 || probeVal == 99 || probeVal == 999) {
            rowsPerChunk = probeVal;
        }
    }
