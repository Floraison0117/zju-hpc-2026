...
extern "C" __global__ __aicore__ void fused_add_rms_norm(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
    ...
    // OOB SAFETY TEST: issue a fixed 8 KB weight load BEFORE any tiling read.
    // For H < 4096 this deliberately over-reads the weight tensor.
    {
        AscendC::TPipe pipeW;
        AscendC::TBuf<AscendC::TPosition::VECCALC> wBuf;
        pipeW.InitBuffer(wBuf, 8 * 1024);
        AscendC::LocalTensor<half> wL = wBuf.Get<half>();
        AscendC::GlobalTensor<half> wG;
        wG.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(weight), 1);
        AscendC::DataCopy(wL, wG[0], 4096);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
        volatile half chk = wL.GetValue(0);
        (void)chk;
    }
