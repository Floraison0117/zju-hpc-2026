...
GPUManager::~GPUManager() {
    ...
    delete pimpl;
    // Reset device context so a subsequent process (e.g. the OJ judge's
    // verification re-run in the same container) inherits a clean CUDA state.
    // Safe: streams already destroyed above and clear_pool() is a no-op for the
    // unused memory pool, so no CUDA_CHECK path can fire after this point.
    cudaDeviceReset();
}
...
void GPUManager::synchronize_all_streams() {
    for (int i = 0; i < Impl::NUM_STREAMS; ++i) CUDA_CHECK(cudaStreamSynchronize(pimpl->stream_pool[i]));
}
