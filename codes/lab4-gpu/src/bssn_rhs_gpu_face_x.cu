#define RHSFACE_AXIS_X
#define RHSFACE_PRED4
#include "bssn_rhs.h"
...
__host__ __device__ static inline int bnd_region_size(int r, int ex0, int ex1, int ex2) {
...
__global__ __launch_bounds__(512, 1) void rhs_kernel_face_x(
    ...
    // Iter26b face kernel: 1-D grid over a single boundary region.
    int i, j, k;
    int n = blockIdx.x * blockDim.x + threadIdx.x;
    int sx0 = bnd_region_size(0, ex0, ex1, ex2);
    int sx1 = bnd_region_size(1, ex0, ex1, ex2);
    int sx = sx0 + sx1;
    if (n >= sx) return;
    int face_region = (n >= sx0) ? 1 : 0;
    if (face_region == 1) n -= sx0;
    bnd_map_region(n, face_region, ex0, ex1, ex2, &i, &j, &k);
    (void)skip_interior;

    int idx = IDX3D(i, j, k, ex0, ex1, ex2);
    const double face_vx = betax[idx];
    const double face_vy = betay[idx];
    const double face_vz = betaz[idx];
    int dims[3] = {ex0, ex1, ex2}; // 用于传给 device 函数
    // Cache coordinate and shift values shared by the face derivative calls.
    double Xfast[2] = {X[0], X[1]};
    double Yfast[2] = {Y[0], Y[1]};
    double Zfast[2] = {Z[0], Z[1]};
#define X Xfast
#define Y Yfast
#define Z Zfast
    ...
    gxx_rhs[idx] += d_lopsided_point_cached(dims, dxx, gxx_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) gxx_rhs[idx] += d_kodis_point(dims, dxx, X, Y, Z, SYM, SYM, SYM, symmetry, eps, i, j, k);

    // gxy (AAS)
    gxy_rhs[idx] += d_lopsided_point_cached(dims, gxy, gxy_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, ANTI, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) gxy_rhs[idx] += d_kodis_point(dims, gxy, X, Y, Z, ANTI, ANTI, SYM, symmetry, eps, i, j, k);

    // gxz (ASA)
    gxz_rhs[idx] += d_lopsided_point_cached(dims, gxz, gxz_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, SYM, ANTI, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) gxz_rhs[idx] += d_kodis_point(dims, gxz, X, Y, Z, ANTI, SYM, ANTI, symmetry, eps, i, j, k);

    // gyy (SSS)
    gyy_rhs[idx] += d_lopsided_point_cached(dims, dyy, gyy_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) gyy_rhs[idx] += d_kodis_point(dims, dyy, X, Y, Z, SYM, SYM, SYM, symmetry, eps, i, j, k);

    // gyz (SAA)
    gyz_rhs[idx] += d_lopsided_point_cached(dims, gyz, gyz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, ANTI, ANTI, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) gyz_rhs[idx] += d_kodis_point(dims, gyz, X, Y, Z, SYM, ANTI, ANTI, symmetry, eps, i, j, k);

    // gzz (SSS)
    gzz_rhs[idx] += d_lopsided_point_cached(dims, dzz, gzz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    ...
    // Axx (SSS)
    Axx_rhs[idx] += d_lopsided_point_cached(dims, Axx, Axx_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Axx_rhs[idx] += d_kodis_point(dims, Axx, X, Y, Z, SYM, SYM, SYM, symmetry, eps, i, j, k);

    // Axy (AAS)
    Axy_rhs[idx] += d_lopsided_point_cached(dims, Axy, Axy_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, ANTI, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Axy_rhs[idx] += d_kodis_point(dims, Axy, X, Y, Z, ANTI, ANTI, SYM, symmetry, eps, i, j, k);

    // Axz (ASA)
    Axz_rhs[idx] += d_lopsided_point_cached(dims, Axz, Axz_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, SYM, ANTI, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Axz_rhs[idx] += d_kodis_point(dims, Axz, X, Y, Z, ANTI, SYM, ANTI, symmetry, eps, i, j, k);

    // Ayy (SSS)
    Ayy_rhs[idx] += d_lopsided_point_cached(dims, Ayy, Ayy_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Ayy_rhs[idx] += d_kodis_point(dims, Ayy, X, Y, Z, SYM, SYM, SYM, symmetry, eps, i, j, k);

    // Ayz (SAA)
    Ayz_rhs[idx] += d_lopsided_point_cached(dims, Ayz, Ayz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, ANTI, ANTI, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Ayz_rhs[idx] += d_kodis_point(dims, Ayz, X, Y, Z, SYM, ANTI, ANTI, symmetry, eps, i, j, k);

    // Azz (SSS)
    Azz_rhs[idx] += d_lopsided_point_cached(dims, Azz, Azz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    ...
    // chi (SSS)
    chi_rhs[idx] += d_lopsided_point_cached(dims, chi, chi_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) chi_rhs[idx] += d_kodis_point(dims, chi, X, Y, Z, SYM, SYM, SYM, symmetry, eps, i, j, k);

    // trK (SSS)
    trK_rhs[idx] += d_lopsided_point_cached(dims, trK, trK_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    ...
    // Gamx (ASS)
    Gamx_rhs[idx] += d_lopsided_point_cached(dims, Gamx, Gamx_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Gamx_rhs[idx] += d_kodis_point(dims, Gamx, X, Y, Z, ANTI, SYM, SYM, symmetry, eps, i, j, k);

    // Gamy (SAS)
    Gamy_rhs[idx] += d_lopsided_point_cached(dims, Gamy, Gamy_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, ANTI, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Gamy_rhs[idx] += d_kodis_point(dims, Gamy, X, Y, Z, SYM, ANTI, SYM, symmetry, eps, i, j, k);

    // Gamz (SSA)
    Gamz_rhs[idx] += d_lopsided_point_cached(dims, Gamz, Gamz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, ANTI, i, j, k, face_vx, face_vy, face_vz);
    ...
    Lap_rhs[idx] += d_lopsided_point_cached(dims, Lap, Lap_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) Lap_rhs[idx] += d_kodis_point(dims, Lap, X, Y, Z, SYM, SYM, SYM, symmetry, eps, i, j, k);

    // betax (ASS)
    betax_rhs[idx] += d_lopsided_point_cached(dims, betax, betax_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) betax_rhs[idx] += d_kodis_point(dims, betax, X, Y, Z, ANTI, SYM, SYM, symmetry, eps, i, j, k);

    // betay (SAS)
    betay_rhs[idx] += d_lopsided_point_cached(dims, betay, betay_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, ANTI, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) betay_rhs[idx] += d_kodis_point(dims, betay, X, Y, Z, SYM, ANTI, SYM, symmetry, eps, i, j, k);

    // betaz (SSA)
    betaz_rhs[idx] += d_lopsided_point_cached(dims, betaz, betaz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, ANTI, i, j, k, face_vx, face_vy, face_vz);
    ...
    // dtSfx (ASS)
    dtSfx_rhs[idx] += d_lopsided_point_cached(dims, dtSfx, dtSfx_rhs, betax, betay, betaz, X, Y, Z, symmetry, ANTI, SYM, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) dtSfx_rhs[idx] += d_kodis_point(dims, dtSfx, X, Y, Z, ANTI, SYM, SYM, symmetry, eps, i, j, k);

    // dtSfy (SAS)
    dtSfy_rhs[idx] += d_lopsided_point_cached(dims, dtSfy, dtSfy_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, ANTI, SYM, i, j, k, face_vx, face_vy, face_vz);
    if (eps > 0.0) dtSfy_rhs[idx] += d_kodis_point(dims, dtSfy, X, Y, Z, SYM, ANTI, SYM, symmetry, eps, i, j, k);

    // dtSfz (SSA)
    dtSfz_rhs[idx] += d_lopsided_point_cached(dims, dtSfz, dtSfz_rhs, betax, betay, betaz, X, Y, Z, symmetry, SYM, SYM, ANTI, i, j, k, face_vx, face_vy, face_vz);
