...
#include <complex>
#include <vector>
#ifdef AMSS_ENABLE_PACKED_RELAX
#include "packed_relax.h"
#endif
using namespace std;
#else
#include <assert.h>
...
/*--------------------------------------------------------------------------*/
#ifdef AMSS_ENABLE_TWOP_COS_TABLE
#include <unordered_map>
#include <vector>
/* Cached Chebyshev cosine tables keyed by transform length n.
 * chebft_Zeros recomputes cos(Pion*j*(k+0.5)) and cos(Pion*(j+0.5)*k) for the
 * same fixed n on every call (n1/n2 are constant for the whole solve).  The
 * table stores the exact double that cos() returns for the same argument, so
 * the serial accumulation sum += u[k]*cos_val is bit-identical to the
 * original; only the repeated cos() evaluation is elided. */
static const std::vector<double> &twop_cos0_table(int n)
{
  ...
  for (int j = 0; j < n; ++j)
    for (int k = 0; k < n; ++k)
      t[(size_t)j * n + k] = cos((Pion * (double)j) * ((double)k + 0.5));
  return cache.emplace(n, std::move(t)).first->second;
}
static const std::vector<double> &twop_cos1_table(int n)
{
  static std::unordered_map<int, std::vector<double>> cache;
  auto it = cache.find(n);
  if (it != cache.end()) return it->second;
  ...
  for (int j = 0; j < n; ++j)
    for (int k = 0; k < n; ++k)
      t[(size_t)j * n + k] = cos((Pion * ((double)j + 0.5)) * (double)k);
  return cache.emplace(n, std::move(t)).first->second;
}
#endif
/*--------------------------------------------------------------------------*/
...
/* eq. 5.8.7 and 5.8.8 at x = (5.8.4) of 2nd edition C++ NR */
{
  ...
  if (inv == 0)
  {
    ...
    for (j = 0; j < n; j++)
    {
      ...
      for (k = 0; k < n; k++)
#ifdef AMSS_ENABLE_TWOP_COS_TABLE
        sum += u[k] * twop_cos0_table(n)[(size_t)j * n + k];
#else
  ...
  else
  {
    for (j = 0; j < n; j++)
    {
      ...
      {
#ifdef AMSS_ENABLE_TWOP_COS_TABLE
        sum += u[k] * twop_cos1_table(n)[(size_t)j * n + k] * isignum;
#else
...
{
  int i, j, k, ivar;
#ifndef AMSS_ENABLE_TWOP_OMP_TUNE
  int _N_dab3 = maximum3(n1, n2, n3);
  double *_p = dvector(0, _N_dab3), *_dp = dvector(0, _N_dab3), *_d2p = dvector(0, _N_dab3);
  double *_q = dvector(0, _N_dab3), *_dq = dvector(0, _N_dab3), *_r = dvector(0, _N_dab3), *_dr = dvector(0, _N_dab3);
  int *_indx = ivector(0, _N_dab3);
#endif

  for (ivar = 0; ivar < nvar; ivar++)
  {
#ifdef AMSS_ENABLE_TWOP_OMP_TUNE
    ...
    for (k = 0; k < n3; k++)
    {
      ...
      {
#ifdef AMSS_ENABLE_TWOP_OMP_TUNE
        double p[64], dp[64], d2p[64];
        int indx[64];
#else
        double *p = _p, *dp = _dp, *d2p = _d2p;
        int *indx = _indx;
#endif
        for (i = 0; i < n1; i++)
        {
    ...
    for (k = 0; k < n3; k++)
    {
      ...
      {
#ifdef AMSS_ENABLE_TWOP_OMP_TUNE
        double p[64], dp[64], d2p[64], q[64], dq[64];
        int indx[64];
#else
        double *p = _p, *dp = _dp, *d2p = _d2p, *q = _q, *dq = _dq;
        int *indx = _indx;
#endif
        for (j = 0; j < n2; j++)
        {
    ...
    for (i = 0; i < n1; i++)
    {
      ...
      {
#ifdef AMSS_ENABLE_TWOP_OMP_TUNE
        double p[64], dp[64], d2p[64], q[64], dq[64], r[64], dr[64];
        int indx[64];
#else
        double *p = _p, *dp = _dp, *d2p = _d2p, *q = _q, *dq = _dq, *r = _r, *dr = _dr;
        int *indx = _indx;
#endif
        for (k = 0; k < n3; k++)
        {
...
#ifndef AMSS_ENABLE_TWOP_OMP_TUNE
  free_dvector(_p, 0, _N_dab3);
  free_dvector(_dp, 0, _N_dab3);
  free_dvector(_d2p, 0, _N_dab3);
  free_dvector(_q, 0, _N_dab3);
  free_dvector(_dq, 0, _N_dab3);
  free_dvector(_r, 0, _N_dab3);
  free_dvector(_dr, 0, _N_dab3);
  free_ivector(_indx, 0, _N_dab3);
#endif
}
/* --------------------------------------------------------------------------*/
                          ...
                          derivs u)
{
  ...
  Derivatives_AB3(nvar, n1, n2, n3, v);
#ifdef USE_GPU
  gpu_F_of_v(nvar, n1, n2, n3,
             v.d0, v.d1, v.d2, v.d3, v.d11, v.d12, v.d13,
             v.d22, v.d23, v.d33,
             F,
             u.d0, u.d1, u.d2, u.d3, u.d11, u.d12, u.d13,
             u.d22, u.d23, u.d33,
             par_b, par_m_plus, par_m_minus,
...
#ifdef AMSS_ENABLE_PACKED_RELAX
  amss_packed_relax::Storage packed_relax(nvar, n1, n2, n3, ncols, cols, JFD);
  amss_packed_relax::Scope packed_relax_scope(&packed_relax);
#endif
...
#ifdef AMSS_ENABLE_TWOP_OMP_TUNE
  /* Hoist a single OpenMP team AND collapse the k+i (or k+j) loops so each
   * pass exposes n3/2 x n1/2 (or n2/2) ~= 325 independent lines instead of
   * ~25, saturating 60 cores.  BIT-EXACT: the 3D stencil couples k to k+/-1
   * only (nearest neighbour), so even-k lines never read each other's outputs.
   * Verified: OMP=1 == OMP=30 == OMP=60 produce identical Ansorg.psid hash. */
#pragma omp parallel
                                ...
                                double **JFD)
{
  ...
  static thread_local std::vector<double> _diag, _e, _f, _b, _x;
  if ((int)_diag.size() != n2)
  {
    _diag.resize(n2); _e.resize(n2 > 0 ? n2 - 1 : 0); _f.resize(n2 > 0 ? n2 - 1 : 0);
    _b.resize(n2); _x.resize(n2);
  }
  double *diag = _diag.data(), *e = _e.data(), *f = _f.data(), *b = _b.data(), *x = _x.data();

#ifdef AMSS_ENABLE_PACKED_RELAX
  if (const amss_packed_relax::Storage *packed = amss_packed_relax::active())
  {
    const amss_packed_relax::Direction &direction = packed->be;
    for (ivar = 0; ivar < nvar; ++ivar)
    {
      for (j = 0; j < n2; ++j)
      {
        ...
        b[j] = rhs[Ic];
        for (int entry = direction.row_offsets[Ic];
             entry < direction.row_offsets[Ic + 1]; ++entry)
        {
#ifdef AMSS_ENABLE_TWOP_OMP_TUNE
          if (entry + 8 < direction.row_offsets[Ic + 1])
            __builtin_prefetch(&dv[direction.columns[entry + 8]], 0, 1);
#endif
          b[j] -= direction.values[entry] * dv[direction.columns[entry]];
        }
        diag[j] = direction.diagonal[Ic];
        if (j > 0)
          f[j - 1] = direction.lower[Ic];
        if (j < n2 - 1)
          e[j] = direction.upper[Ic];
      }
                                ...
                                int **cols, double **JFD)
{
  ...
  static thread_local std::vector<double> _diag, _e, _f, _b, _x;
  if ((int)_diag.size() != n1)
  {
    _diag.resize(n1); _e.resize(n1 > 0 ? n1 - 1 : 0); _f.resize(n1 > 0 ? n1 - 1 : 0);
    _b.resize(n1); _x.resize(n1);
  }
  double *diag = _diag.data(), *e = _e.data(), *f = _f.data(), *b = _b.data(), *x = _x.data();
  ...
  if (const amss_packed_relax::Storage *packed = amss_packed_relax::active())
  {
    const amss_packed_relax::Direction &direction = packed->al;
    for (ivar = 0; ivar < nvar; ++ivar)
    {
      for (i = 0; i < n1; ++i)
      {
             ...
             entry < direction.row_offsets[Ic + 1]; ++entry)
        {
          ...
          if (entry + 8 < direction.row_offsets[Ic + 1])
            __builtin_prefetch(&dv[direction.columns[entry + 8]], 0, 1);
#endif
          b[i] -= direction.values[entry] * dv[direction.columns[entry]];
        }
        diag[i] = direction.diagonal[Ic];
        if (i > 0)
          f[i - 1] = direction.lower[Ic];
        if (i < n1 - 1)
          e[i] = direction.upper[Ic];
      }
...
{
  int i;
  static thread_local std::vector<double> _l, _u, _d, _y;
  if ((int)_d.size() != N)
  {
    _d.resize(N); _y.resize(N); _l.resize(N > 0 ? N - 1 : 0); _u.resize(N > 0 ? N - 1 : 0);
  }
  double *l = _l.data(), *u = _u.data(), *d = _d.data(), *y = _y.data();
