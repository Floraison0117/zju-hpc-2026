...
# --- Default GPU decouple config (reproduces measured 1271.50s OJ-equiv) ---
# TwoPunctureABE builds CPU+OpenMP (fast ~34s path, AMSS_ENABLE_TWOP_GPU=OFF);
# ABEGPU builds GPU. Override any by setting the env var before invoking.
: "${AMSS_ENABLE_GPU:=ON}"
: "${AMSS_ENABLE_TWOP_GPU:=OFF}"
: "${AMSS_ENABLE_TWOP_OMP_TUNE:=ON}"
: "${AMSS_ENABLE_PACKED_RELAX:=ON}"
: "${AMSS_ENABLE_TWOP_COS_TABLE:=ON}"
: "${AMSS_OPT:=-Ofast -fno-frontend-optimize}"
export AMSS_ENABLE_GPU AMSS_ENABLE_TWOP_GPU AMSS_ENABLE_TWOP_OMP_TUNE AMSS_ENABLE_PACKED_RELAX AMSS_ENABLE_TWOP_COS_TABLE AMSS_OPT
...
[[ -n "${AMSS_ENABLE_PACKED_RELAX:-}" ]] && cmake_args+=("-DAMSS_ENABLE_PACKED_RELAX=$AMSS_ENABLE_PACKED_RELAX")
[[ -n "${AMSS_ENABLE_TWOP_COS_TABLE:-}" ]] && cmake_args+=("-DAMSS_ENABLE_TWOP_COS_TABLE=$AMSS_ENABLE_TWOP_COS_TABLE")
[[ -n "${AMSS_ENABLE_TWOP_GPU_ASYNC:-}" ]] && cmake_args+=("-DAMSS_ENABLE_TWOP_GPU_ASYNC=$AMSS_ENABLE_TWOP_GPU_ASYNC")
[[ -n "${AMSS_ENABLE_TWOP_GPU:-}" ]]        && cmake_args+=("-DAMSS_ENABLE_TWOP_GPU=$AMSS_ENABLE_TWOP_GPU")
[[ -n "${AMSS_ENABLE_TWOP_OMP_TUNE:-}" ]]   && cmake_args+=("-DAMSS_ENABLE_TWOP_OMP_TUNE=$AMSS_ENABLE_TWOP_OMP_TUNE")
[[ -n "${AMSS_ENABLE_PACKED_RELAX:-}" ]]    && cmake_args+=("-DAMSS_ENABLE_PACKED_RELAX=$AMSS_ENABLE_PACKED_RELAX")
