...
#if TRSM_SOLVE_ILP >= 2
            /* 2-row ILP: rows i and i+1 share the B[k] loads */
            int i = 0;
            for (; i + 1 < bs; i += 2) {
                const double* restrict Li  = Lbase + (size_t)i * lda;
                const double* restrict Li1 = Lbase + (size_t)(i + 1) * lda;
                double* restrict Bi  = Bbase + (size_t)i * ldb;
                double* restrict Bi1 = Bbase + (size_t)(i + 1) * ldb;
                double s1[32], s2[32];
                for (int jj = 0; jj < w; ++jj) { s1[jj] = Bi[jj]; s2[jj] = Bi1[jj]; }

                const double* restrict Bk = Bbase;
                for (int k = 0; k < i; ++k) {
                    const double l1 = Li[k], l2 = Li1[k];
#pragma omp simd
...
#pragma omp simd
                    for (int jj = 0; jj < w; ++jj) Bi[jj] = s1[jj] * inv;
                }
                /* row i+1: include k = i term, then normalize */
                {
                    const double lik = Li1[i];
                    const double inv = 1.0 / Li1[i + 1];
#pragma omp simd
                    for (int jj = 0; jj < w; ++jj)
                        Bi1[jj] = (s2[jj] - lik * Bi[jj]) * inv;
                }
            }
            for (; i < bs; ++i) {
                const double* restrict Li = Lbase + (size_t)i * lda;
                double* restrict Bi = Bbase + (size_t)i * ldb;
                double s1[32];
                for (int jj = 0; jj < w; ++jj) s1[jj] = Bi[jj];
                const double* restrict Bk = Bbase;
                for (int k = 0; k < i; ++k) {
                    const double lik = Li[k];
#pragma omp simd
                    for (int jj = 0; jj < w; ++jj) s1[jj] -= lik * Bk[jj];
                    Bk += ldb;
                }
                const double inv = 1.0 / Li[i];
#pragma omp simd
...
#if defined(__aarch64__) && !defined(TRSM_SCALAR_PACK)
            /* vectorised transpose: two 8-lane SVE gathers per k-value */
            {
