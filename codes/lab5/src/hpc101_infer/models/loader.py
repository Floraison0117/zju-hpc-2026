...
def load_gemma4(
    ...
    if linear_backend not in {"bf16", "int4_reference", "int4_fused", "int4_hybrid"}:
    ...
    if weight_offload not in {"none", "sync", "async"}:
        raise ValueError("weight_offload must be none, sync, or async")
    if weight_offload != "none" and (not is_quantized or device.type != "cuda"):
        raise ValueError("weight offloading requires a quantized checkpoint on CUDA")

    if is_quantized:
        source = QuantizedCheckpointSource(model_path)
        scale_dtype = SCALE_DTYPES[source.quantization_config.scale_dtype]
        if linear_backend == "int4_hybrid":
            from hpc101_infer.layers.hybrid_linear import HybridQuantizedLinearFactory
            linear_factory = HybridQuantizedLinearFactory(
                source.manifest, scale_dtype=scale_dtype
            )
        elif linear_backend == "int4_fused":
            from hpc101_infer.layers.triton_linear import FusedQuantizedLinearFactory
            linear_factory = FusedQuantizedLinearFactory(
                source.manifest, scale_dtype=scale_dtype
            )
        else:
    ...
    with torch.device("meta"):
        model = Gemma4ForCausalLM(
            config, linear_factory=linear_factory,
    ...
    for checkpoint_key in source.keys():
        ...
        if (
            weight_offload != "none"
            and local_key.startswith("layers.")
            and local_key.endswith((".qweight", ".scales", ".zeros"))
        ):
    ...
    if weight_offload != "none":
        from hpc101_infer.runtime.weight_offload import LayerWeightOffloader

        model.weight_offloader = LayerWeightOffloader(
            model.layers,
            device=device,
            mode=weight_offload,
            # async 模式下权重直接驻留 GPU（packed INT4 全模型 ~2.9GiB 放得下），
            # 消除每步 48 层 H2D（profiler：H2D 占 CUDA 时间 48.8%）。
            resident=(weight_offload == "async"),
        )
    return model.eval()
