...
from hpc101_infer.layers.triton_attention import flash_attention
...
class AttentionLayer(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            kv_cache.write(layer_id, position_ids, key, value, slot_ids=slot_ids)
            cached = kv_cache.view(layer_id, max_seq_len, ring_indexed=(self.kv_cache_backend == "ring_indexed" and seq_len == 1), slot_ids=slot_ids)
            key, value = cached.key.clone(), cached.value.clone()
        if self.attention_backend == "flash":
            output = flash_attention(
                query, key, value, position_ids, sequence_lengths,
                layer_type="full_attention",
                ring_indexed=(self.kv_cache_backend == "ring_indexed" and seq_len == 1),
                logical_k_len=max_seq_len if (self.kv_cache_backend == "ring_indexed" and seq_len == 1) else None,
            )
        else:
...
class SlidingAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        window_active = (
            ...
            and kv_cache.layers[layer_id].window_size > 0
        )
        ring_indexed_flag = False
        logical_k_len = None
        if kv_cache is not None:
            if window_active and seq_len > 1:
                # 长 prompt 的一次性 prefill：直接用本层刚投影出的完整 K/V 做滑窗
                # 注意力，结果与 dense 逐位一致；只为后续 decode 把最后 sliding_window
                # 个位置写入环形缓冲。环形容量小于 prompt 长度时全量写入会按取模覆盖
                # 并破坏早期位置，故只写尾部窗口。
                window = kv_cache.layers[layer_id].window_size
                if seq_len <= window:
                    kv_cache.write(layer_id, position_ids, key, value, slot_ids=slot_ids)
                else:
                ...
                # key/value 保持为原始完整张量，不读取缓存。
            elif window_active and seq_len == 1:
                # decode：新 token 写入环形槽 position % window，按 ring_indexed 读取
                # 物理窗口，由 kernel 做取模与滑窗掩码。
                kv_cache.write(layer_id, position_ids, key, value, slot_ids=slot_ids)
                cached = kv_cache.view(layer_id, max_seq_len, ring_indexed=True, slot_ids=slot_ids)
                key, value = cached.key.clone(), cached.value.clone()
                ring_indexed_flag = True
                logical_k_len = max_seq_len
            else:
                # dense（任务一精度评测路径）：保持原行为。
                kv_cache.write(layer_id, position_ids, key, value, slot_ids=slot_ids)
                cached = kv_cache.view(layer_id, max_seq_len, ring_indexed=(self.kv_cache_backend == "ring_indexed" and seq_len == 1), slot_ids=slot_ids)
                key, value = cached.key.clone(), cached.value.clone()
                ring_indexed_flag = self.kv_cache_backend == "ring_indexed" and seq_len == 1
                logical_k_len = max_seq_len if ring_indexed_flag else None
        # ring_indexed decode 必须走 flash kernel（取模寻址）；其余沿用配置后端。
        if self.attention_backend == "flash" or ring_indexed_flag:
            output = flash_attention(
                query, key, value, position_ids, sequence_lengths,
                layer_type="sliding_attention",
                sliding_window=self.sliding_window,
                ring_indexed=ring_indexed_flag,
                logical_k_len=logical_k_len,
            )
        else:
            key = repeat_kv(key, self.num_kv_groups)
            value = repeat_kv(value, self.num_kv_groups)
