...
class Runner:
    ...
    def drain_continuous(
        ...
        *,
        ...
        outputs = self.engine.generate_continuous(requests)
        if len(outputs) != len(batch):
            raise RuntimeError("engine output count does not match batch size")
        completed = []
        for item, output in zip(batch, outputs, strict=True):
    ...
    def run(
        ...
        *,
        ...
        handles = []
        scheduler = self.engine.config.scheduler_backend
        if scheduler in ("rebind_batch", "continuous_batch"):
            for request in requests:
                handles.append(self.submit(request))
            self.drain_continuous(on_completed=on_completed)
        else:
            for request in requests:
                handles.append(self.submit(request))
