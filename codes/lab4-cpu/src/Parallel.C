...
#if defined(AMSS_ENABLE_LOADBAL) || defined(AMSS_LOADBAL_PROFILE)
// ---- load-balance state (AMSS_ENABLE_LOADBAL / AMSS_LOADBAL_PROFILE) ----
// File-scope cumulative per-rank point count. reset_dist_load() zeroes it
// at the start of each compose/recompose pass so load carries across levels.
static long long *dist_load_ = nullptr;
static int dist_load_sz_ = 0;

void Parallel::reset_dist_load(int nprocs)
{
    if (nprocs <= 0)
        return;
    if (nprocs > dist_load_sz_)
    {
        delete[] dist_load_;
        dist_load_ = new long long[nprocs];
        dist_load_sz_ = nprocs;
    }
...
#if defined(AMSS_ENABLE_LOADBAL)
    // Lazy-init: reset_dist_load is normally called by compose_cgh, but guard
    // in case distribute() is reached by another path.
    if (dist_load_ == nullptr || dist_load_sz_ < cpusize)
        reset_dist_load(cpusize);
#endif
        ...
        for (int i = 0; i < nxyz[0]; i++)
            for (int j = 0; j < nxyz[1]; j++)
                for (int k = 0; k < nxyz[2]; k++)
                {
                    ...
                    {
                        int best_rank = 0;
                        for (int rr = 1; rr < cpusize; rr++)
                            if (dist_load_[rr] < dist_load_[best_rank])
                                best_rank = rr;
                        ng = ng0 = new Block(dim, shape_here, bbox_here, best_rank, ingfsi, fngfsi, PP->lev); // delete through KillBlocks
                        long long blk_pts = 1;
                        for (int dd = 0; dd < dim; dd++)
                            blk_pts *= (long long)shape_here[dd];
                        dist_load_[best_rank] += blk_pts;
                    }
#else
                    ng = ng0 = new Block(dim, shape_here, bbox_here, n_rank++, ingfsi, fngfsi, PP->lev); // delete through KillBlocks
#if defined(AMSS_LOADBAL_PROFILE)
                    {
                        long long blk_pts = 1;
                        for (int dd = 0; dd < dim; dd++)
                            blk_pts *= (long long)shape_here[dd];
                        dist_load_[n_rank == 0 ? 0 : n_rank - 1] += blk_pts;
                    }
#endif
#endif
                    //	    ng->checkBlock();
                    ...
                    {
                        int myrank_pr;
                        MPI_Comm_rank(MPI_COMM_WORLD, &myrank_pr);
                        long long my_pts = 0;
                        for (int rr = 0; rr < cpusize; rr++)
                            if (rr == myrank_pr)
                                my_pts = dist_load_[rr];
                        fprintf(stderr, "LOADBAL_PROFILE rank=%d pts=%lld\n", myrank_pr, my_pts);
                    }
#endif
