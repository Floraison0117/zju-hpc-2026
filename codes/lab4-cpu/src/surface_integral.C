...
#define PI M_PI
#ifdef AMSS_ENABLE_ANGULAR_CACHE
#define AMSS_SURFACE_QUAD_WEIGHT(N, I) quad_weight_g[N]
#else
#define AMSS_SURFACE_QUAD_WEIGHT(N, I) wtcostheta[I]
#endif
//|============================================================================
//| Constructor
//|============================================================================
...
#ifdef AMSS_ENABLE_ANGULAR_CACHE
    quad_weight_g = new double[n_tot];
    wave_theta_cache = NULL;
    wave_cos_cache = NULL;
    wave_sin_cache = NULL;
    wave_cache_spinw = wave_cache_maxl = wave_cache_NN = -1;
    wave_cache_Nmin = 0;
    wave_cache_Nmax = -1;
    wave_cache_lpsy = 0;
#endif
...
surface_integral::~surface_integral() {
    ...
    delete[] wtcostheta;
#ifdef AMSS_ENABLE_ANGULAR_CACHE
    delete[] quad_weight_g;
    delete[] wave_theta_cache;
    delete[] wave_cos_cache;
    delete[] wave_sin_cache;
#endif
#ifdef USE_GPU
...
#ifdef AMSS_ENABLE_ANGULAR_CACHE
void surface_integral::ensure_wave_angular_cache(int spinw, int maxl, int NN,
                                                 int Nmin, int Nmax, int lpsy) {
    if (wave_theta_cache != NULL && wave_cache_spinw == spinw &&
        wave_cache_maxl == maxl && wave_cache_NN == NN &&
        wave_cache_Nmin == Nmin && wave_cache_Nmax == Nmax &&
        wave_cache_lpsy == lpsy)
        return;

    delete[] wave_theta_cache;
    delete[] wave_cos_cache;
    delete[] wave_sin_cache;
    wave_theta_cache = wave_cos_cache = wave_sin_cache = NULL;

    const int local_points = Nmax >= Nmin ? Nmax - Nmin + 1 : 0;
    const size_t cache_size = static_cast<size_t>(local_points) * NN * lpsy;
    wave_theta_cache = new double[cache_size];
    wave_cos_cache = new double[cache_size];
    wave_sin_cache = new double[cache_size];
    ...
    for (int n = Nmin; n <= Nmax; ++n) {
        ...
        for (int pl = spinw; pl <= maxl; ++pl) {
            const double normalization = sqrt((2 * pl + 1.0) / 4.0 / PI);
            for (int pm = -pl; pm <= pl; ++pm) {
                for (int lp = 0; lp < lpsy; ++lp) {
                    ...
                    const size_t index =
                        (static_cast<size_t>(n - Nmin) * NN + countlm) * lpsy + lp;
                    wave_theta_cache[index] = normalization *
                        misc::Wigner_d_function(pl, pm, spinw, costheta);
                    wave_cos_cache[index] = cosmphi;
                    wave_sin_cache[index] = sinmphi;
                }
                ++countlm;
            }
        }
    ...
    wave_cache_NN = NN;
    wave_cache_Nmin = Nmin;
    wave_cache_Nmax = Nmax;
    wave_cache_lpsy = lpsy;
}
#endif
//|----------------------------------------------------------------
...
{
    const double profile_total_start = analysis_profile_now();
    analysis_profile_set_phase("surf_Wave.prepare", rex);
    const double profile_prepare_start = analysis_profile_now();
...
#endif
    analysis_profile_record("surf_Wave.prepare", rex, "wall",
                            analysis_profile_now() - profile_prepare_start);
    analysis_profile_set_phase("surf_Wave.interp", rex);
    const double profile_interp_start = analysis_profile_now();
#ifdef AMSS_ENABLE_ANALYSIS_INTERP_BATCH
    GH->PatL[lev]->data->Interp_Points_Analysis_Batch(DG_List, n_tot, pox, shellf, Symmetry);
#else
    GH->PatL[lev]->data->Interp_Points(DG_List, n_tot, pox, shellf, Symmetry);
#endif
    analysis_profile_record("surf_Wave.interp", rex, "wall",
                            analysis_profile_now() - profile_interp_start);
    analysis_profile_set_phase("surf_Wave.local_integration", rex);
    const double profile_local_start = analysis_profile_now();
...
#ifdef AMSS_ENABLE_ANGULAR_CACHE
    ensure_wave_angular_cache(spinw, maxl, NN, Nmin, Nmax, lpsy);
#endif
    ...
    for (n = Nmin; n <= Nmax; n++)
    {
        ...
        for (int pl = spinw; pl < maxl + 1; pl++)
            for (int pm = -pl; pm < pl + 1; pm++)
            {
                ...
                {
#ifdef AMSS_ENABLE_ANGULAR_CACHE
                    const size_t angular_index =
                        (static_cast<size_t>(n - Nmin) * NN + countlm) * lpsy + lp;
                    thetap = wave_theta_cache[angular_index];
                    cosmphi = wave_cos_cache[angular_index];
                    sinmphi = wave_sin_cache[angular_index];
#endif
                    switch (lp)
                    {
                    case 0: //+++ (theta, phi)
#ifndef AMSS_ENABLE_ANGULAR_CACHE
                        costheta = arcostheta[i];
                        cosmphi = cos(pm * (j + 0.5) * dphi);
                        sinmphi = sin(pm * (j + 0.5) * dphi);
#endif
    ...
    analysis_profile_record("surf_Wave.local_integration", rex, "wall",
                            analysis_profile_now() - profile_local_start);
    analysis_profile_set_phase("surf_Wave.collective", rex);
    const double profile_collective_start = analysis_profile_now();
#ifdef AMSS_ENABLE_PACKED_ANALYSIS_COLLECTIVES
    double *wave_out = new double[2 * NN];
    double *wave = new double[2 * NN];
    for (int ii = 0; ii < NN; ++ii)
    {
        wave_out[ii] = RP_out[ii];
        wave_out[NN + ii] = IP_out[ii];
    }
    MPI_Allreduce(wave_out, wave, 2 * NN, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    for (int ii = 0; ii < NN; ++ii)
    {
        RP[ii] = wave[ii];
        IP[ii] = wave[NN + ii];
...
#endif
    analysis_profile_record("surf_Wave.collective", rex, "wall",
                            analysis_profile_now() - profile_collective_start);

    //|------= Free memory.

    analysis_profile_set_phase("surf_Wave.cleanup", rex);
    const double profile_cleanup_start = analysis_profile_now();
    delete[] pox[0];
    delete[] pox[1];
    delete[] pox[2];
    delete[] shellf;
    ...
    delete[] IP_out;
    DG_List->clearList();
    analysis_profile_record("surf_Wave.cleanup", rex, "wall",
                            analysis_profile_now() - profile_cleanup_start);
    analysis_profile_record("surf_Wave.internal", rex, "wall",
                            analysis_profile_now() - profile_total_start);
}
void surface_integral::surf_Wave(double rex, int lev, cgh *GH, var *Rpsi4, var *Ipsi4,
                                                                 int spinw, int maxl, int NN, double *RP, double *IP,
...
{
  const double profile_total_start = analysis_profile_now();
  analysis_profile_set_phase("surf_MassPAng.prepare", rex);
  const double profile_prepare_initial_start = analysis_profile_now();
  if (myrank == 0 && GH->grids[lev] != 1)
  ...
  double profile_prepare_duration = analysis_profile_now() - profile_prepare_initial_start;
  analysis_profile_set_phase("surf_MassPAng.f_admmass_bssn", rex);
  const double profile_adm_start = analysis_profile_now();
  ...
  analysis_profile_record("surf_MassPAng.f_admmass_bssn", rex, "wall",
                          analysis_profile_now() - profile_adm_start);
  analysis_profile_set_phase("surf_MassPAng.prepare", rex);
  const double profile_prepare_final_start = analysis_profile_now();
  ...
  profile_prepare_duration += analysis_profile_now() - profile_prepare_final_start;
  analysis_profile_record("surf_MassPAng.prepare", rex, "wall",
                          profile_prepare_duration);
  analysis_profile_set_phase("surf_MassPAng.interp", rex);
  const double profile_interp_start = analysis_profile_now();
#ifdef AMSS_ENABLE_ANALYSIS_INTERP_BATCH
  GH->PatL[lev]->data->Interp_Points_Analysis_Batch(DG_List, n_tot, pox, shellf, Symmetry);
#else
  GH->PatL[lev]->data->Interp_Points(DG_List, n_tot, pox, shellf, Symmetry);
#endif
  analysis_profile_record("surf_MassPAng.interp", rex, "wall",
                          analysis_profile_now() - profile_interp_start);
  analysis_profile_set_phase("surf_MassPAng.local_integration", rex);
  const double profile_local_start = analysis_profile_now();
...
// but this factor has been considered in f_admmass_bssn
    // wtcostheta is even function respect costheta
    Mass_out = Mass_out + (shellf[InList * n] * nx_g[n] + shellf[InList * n + 1] * ny_g[n] + shellf[InList * n + 2] * nz_g[n]) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
    ...
    if (Symmetry == 0)
    {
      ...
      ang_outx = ang_outx + f1o8 * Psi * (nx_g[n] * (pox[1][n] * aupxz - pox[2][n] * aupxy) + ny_g[n] * (pox[1][n] * aupyz - pox[2][n] * aupyy) + nz_g[n] * (pox[1][n] * aupzz - pox[2][n] * aupzy)) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
      //  1/8\pi \int \psi^6 (z A^m_x - xA^m_z) dS_m
      ang_outy = ang_outy + f1o8 * Psi * (nx_g[n] * (pox[2][n] * aupxx - pox[0][n] * aupxz) + ny_g[n] * (pox[2][n] * aupyx - pox[0][n] * aupyz) + nz_g[n] * (pox[2][n] * aupzx - pox[0][n] * aupzz)) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
      // 1/8\pi \int \psi^6 (x A^m_y - yA^m_x) dS_m
      ang_outz = ang_outz + f1o8 * Psi * (nx_g[n] * (pox[0][n] * aupxy - pox[1][n] * aupxx) + ny_g[n] * (pox[0][n] * aupyy - pox[1][n] * aupyx) + nz_g[n] * (pox[0][n] * aupzy - pox[1][n] * aupzx)) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
    }
    else if (Symmetry == 1)
    {
      ang_outz = ang_outz + f1o8 * Psi * (nx_g[n] * (pox[0][n] * aupxy - pox[1][n] * aupxx) + ny_g[n] * (pox[0][n] * aupyy - pox[1][n] * aupyx) + nz_g[n] * (pox[0][n] * aupzy - pox[1][n] * aupzx)) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
    ...
    if (Symmetry == 0)
    {
      p_outx = p_outx + f1o8 * Psi * (nx_g[n] * axx + ny_g[n] * axy + nz_g[n] * axz) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
      p_outy = p_outy + f1o8 * Psi * (nx_g[n] * axy + ny_g[n] * ayy + nz_g[n] * ayz) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
      p_outz = p_outz + f1o8 * Psi * (nx_g[n] * axz + ny_g[n] * ayz + nz_g[n] * azz) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
    }
    else if (Symmetry == 1)
    {
      p_outx = p_outx + f1o8 * Psi * (nx_g[n] * axx + ny_g[n] * axy + nz_g[n] * axz) * AMSS_SURFACE_QUAD_WEIGHT(n, i);
  ...
  analysis_profile_record("surf_MassPAng.local_integration", rex, "wall",
                          analysis_profile_now() - profile_local_start);
  analysis_profile_set_phase("surf_MassPAng.collective", rex);
  const double profile_collective_start = analysis_profile_now();
#ifdef AMSS_ENABLE_PACKED_ANALYSIS_COLLECTIVES
  const double adm_out[7] = {Mass_out, ang_outx, ang_outy, ang_outz,
                             p_outx, p_outy, p_outz};
  double adm[7];
  MPI_Allreduce(adm_out, adm, 7, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  mass = adm[0];
  sx = adm[1];
  sy = adm[2];
  sz = adm[3];
...
#endif
  analysis_profile_record("surf_MassPAng.collective", rex, "wall",
                          analysis_profile_now() - profile_collective_start);
  analysis_profile_set_phase("surf_MassPAng.cleanup", rex);
  const double profile_cleanup_start = analysis_profile_now();
  ...
  delete[] shellf;
  DG_List->clearList();
  analysis_profile_record("surf_MassPAng.cleanup", rex, "wall",
                          analysis_profile_now() - profile_cleanup_start);
  analysis_profile_record("surf_MassPAng.internal", rex, "wall",
                          analysis_profile_now() - profile_total_start);
}
