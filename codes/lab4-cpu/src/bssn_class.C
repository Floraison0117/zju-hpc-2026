...
void bssn_class::Compute_Psi4(int lev)
{
  ...
  analysis_profile_set_phase("Compute_Psi4.prepare", -1.0);
  const double profile_prepare_start = analysis_profile_now();
  MyList<var> *DG_List = new MyList<var>(Rpsi4);
  DG_List->insert(Ipsi4);

  MyList<Patch> *Pp = GH->PatL[lev];
  analysis_profile_record("Compute_Psi4.prepare", -1.0, "wall",
                          analysis_profile_now() - profile_prepare_start);
  analysis_profile_set_phase("Compute_Psi4.f_getnp4", -1.0);
  const double profile_getnp4_start = analysis_profile_now();
  ...
  analysis_profile_record("Compute_Psi4.f_getnp4", -1.0, "wall",
                          analysis_profile_now() - profile_getnp4_start);
  analysis_profile_set_phase("Compute_Psi4.Parallel::Sync", -1.0);
  const double profile_sync_start = analysis_profile_now();
  Parallel::Sync(GH->PatL[lev], DG_List, Symmetry);
  analysis_profile_record("Compute_Psi4.Parallel::Sync", -1.0, "wall",
                          analysis_profile_now() - profile_sync_start);

  analysis_profile_set_phase("Compute_Psi4.cleanup", -1.0);
  const double profile_cleanup_start = analysis_profile_now();
  DG_List->clearList();
  analysis_profile_record("Compute_Psi4.cleanup", -1.0, "wall",
                          analysis_profile_now() - profile_cleanup_start);
  analysis_profile_record("Compute_Psi4", -1.0, "wall",
...
{
  const double profile_analysis_start = analysis_profile_now();
  analysis_profile_begin(int(PhysTime / dT + 0.5), PhysTime);
  LastAnas += dT_lev;

  // Always output BH position for check.sh trajectory matching
  {
    analysis_profile_set_phase("AnalysisStuff.BH_monitor", -1.0);
    const double profile_bh_monitor_start = analysis_profile_now();
    double *pox;
    ...
    delete[] pox;
    analysis_profile_record("AnalysisStuff.BH_monitor", -1.0, "wall",
                            analysis_profile_now() - profile_bh_monitor_start);
  }

  if (LastAnas >= AnasTime)
  {
    analysis_profile_set_phase("Compute_Psi4", -1.0);
    Compute_Psi4(lev);
    analysis_profile_set_phase("AnalysisStuff.allocation", -1.0);
    const double profile_allocation_start = analysis_profile_now();
    ...
    analysis_profile_record("AnalysisStuff.allocation", -1.0, "wall",
                            analysis_profile_now() - profile_allocation_start);
    double Rex = maxrex;
    for (int i = 0; i < decn; i++)
    {
      const double profile_radius_start = analysis_profile_now();
      analysis_profile_set_phase("surf_Wave", Rex);
      const double profile_wave_start = analysis_profile_now();
      Waveshell->surf_Wave(Rex, lev, GH, Rpsi4, Ipsi4, 2, maxl, NN, RP, IP, ErrorMonitor);
      analysis_profile_record("surf_Wave", Rex, "wall",
                              analysis_profile_now() - profile_wave_start);
      analysis_profile_set_phase("surf_MassPAng", Rex);
      const double profile_mass_start = analysis_profile_now();
      ...
      analysis_profile_record("surf_MassPAng", Rex, "wall",
                              analysis_profile_now() - profile_mass_start);
      if (i == 0)
      {
        ADMMass = RoutMAP[0];
      }
      analysis_profile_set_phase("AnalysisStuff.monitor", Rex);
      const double profile_monitor_start = analysis_profile_now();
      Psi4Monitor->writefile(PhysTime, NN, RP, IP);
      MAPMonitor->writefile(PhysTime, 7, RoutMAP);
      analysis_profile_record("AnalysisStuff.monitor", Rex, "wall",
                              analysis_profile_now() - profile_monitor_start);
      analysis_profile_record("AnalysisStuff.radius", Rex, "wall",
                              analysis_profile_now() - profile_radius_start);
      Rex = Rex - drex;
    }
    analysis_profile_set_phase("AnalysisStuff.cleanup", -1.0);
    const double profile_cleanup_start = analysis_profile_now();
    delete[] RP;
    delete[] IP;
    delete[] RoutMAP;
    LastAnas = 0;
    analysis_profile_record("AnalysisStuff.cleanup", -1.0, "wall",
                            analysis_profile_now() - profile_cleanup_start);
  }
  analysis_profile_set_phase("AnalysisStuff", -1.0);
  analysis_profile_end(profile_analysis_start);
}
...
void bssn_class::Constraint_Out()
{
  ...
  if (true) // Always compute constraints for check.sh, independent of AnasTime
  // Constraint violation
  {
    ...
    for (int lev = 0; lev < GH->levels; lev++)
    {
      ...
      if (false) // SKIPPED: constrait quantities reused from step rhs, check.sh only checks level 0
      {
        double TRK4 = PhysTime;
