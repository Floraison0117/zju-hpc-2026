...
! interior (bulk): 12-point reading f directly, no ghost needed
  do k=3,ex(3)-2
  do j=3,ex(2)-2
  do i=3,ex(1)-2
...
! boundary (thin shell): 12/2-point with inline symmetry reflection
  do k=1,ex(3)-1
  do j=1,ex(2)-1
  do i=1,ex(1)-1
    if(i>=3 .and. i<=ex(1)-2 .and. j>=3 .and. j<=ex(2)-2 .and. k>=3 .and. k<=ex(3)-2) cycle
    if(i+2 <= imax .and. i-2 >= imin .and. j+2 <= jmax .and. j-2 >= jmin .and. k+2 <= kmax .and. k-2 >= kmin) then
      fx(i,j,k)=d12dx*(frx(i-2,j,k)-EIT*frx(i-1,j,k)+EIT*frx(i+1,j,k)-frx(i+2,j,k))
      fy(i,j,k)=d12dy*(fry(i,j-2,k)-EIT*fry(i,j-1,k)+EIT*fry(i,j+1,k)-fry(i,j+2,k))
      fz(i,j,k)=d12dz*(frz(i,j,k-2)-EIT*frz(i,j,k-1)+EIT*frz(i,j,k+1)-frz(i,j,k+2))
    elseif(i+1 <= imax .and. i-1 >= imin .and. j+1 <= jmax .and. j-1 >= jmin .and. k+1 <= kmax .and. k-1 >= kmin) then
      fx(i,j,k)=d2dx*(-frx(i-1,j,k)+frx(i+1,j,k))
      fy(i,j,k)=d2dy*(-fry(i,j-1,k)+fry(i,j+1,k))
      fz(i,j,k)=d2dz*(-frz(i,j,k-1)+frz(i,j,k+1))
    endif
  enddo
  enddo
  enddo
  ...
  contains
    function frx(a,j,k)
      ...
      real*8::frx
      if(a>=1)then; frx=f(a,j,k); else; frx=f(-a+1,j,k)*SoA(1); endif
    end function frx
    function fry(i,b,k)
      implicit none
      integer,intent(in)::i,b,k
      real*8::fry
      if(b>=1)then; fry=f(i,b,k); else; fry=f(i,-b+1,k)*SoA(2); endif
    end function fry
    function frz(i,j,c)
      implicit none
      integer,intent(in)::i,j,c
      real*8::frz
      if(c>=1)then; frz=f(i,j,c); else; frz=f(i,j,-c+1)*SoA(3); endif
    end function frz
