...
def dequantize_weight(
    ...
    *,
    ...
    qw = quantized.qweight
    # Detect layout: [N, K_packed] (original/task1) vs [K_packed, N] (V10 transposed).
    # V10 transposes qweight for coalesced decode dot. For prefill (cuBLAS, needs [N,K]),
    # a .t() VIEW (no copy) restores the [N, K_packed] layout, then the original path runs.
    # task1 (int4_reference, [N,K_packed]) is unchanged: shape[0]==out_features, no .t().
    if qw.shape[0] != out_features:
        qw = qw.t()  # free view [N, K_packed], no materialization
    encoded = unpack_int4(qw, padded_in_features)
