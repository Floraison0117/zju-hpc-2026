...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
    ...
    // leave margin for the framework/stack). Per-row footprint there:
    //   double-buffered in  (x+res FP16): 8*H bytes
    //   R in FP32 kept in UB             : 4*H bytes
    //   double-buffered out (y+res_out)  : 8*H bytes
    // total 20*H bytes per row. Cap rowsPerChunk so every core runs at least
    // two chunks (chunk-level MTE2/V/MTE3 double buffering then has room to
    // overlap). For [256,1024] with 6-7 rows/core, chunk=3 gives 2-3 chunks.
    constexpr int64_t kBatchUbBudget = 144 * 1024;
    ...
    context->SetBlockDim(aivNum < B ? aivNum : B);
