 ...
 *       computed with vector ops on the sumSq array plus one scalar reciprocal
 *       per row; scaling is Muls(R, rstd) + Mul(weight). Outputs are cast to
 *       FP16 chunk-wise and written with one DataCopy each. All TQue double
 *       buffering is at chunk granularity, so the per-row scalar overhead of
 *       the queue API (the dominant cost in the baseline) is amortized away.
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
        ...
        if (this->useBatch) {
            ...
            // two-half pipeline: each inQue buffer holds x|residual of half rows
            uint32_t halfChunkFp16 =
                static_cast<uint32_t>(((chunk + 1) / 2) * this->alignedHidden) * sizeof(half);
            pipe->InitBuffer(inQue, BUFFER_NUM, 2u * halfChunkFp16);
            pipe->InitBuffer(outQueY, BUFFER_NUM, chunkFp16);
            pipe->InitBuffer(outQueResOut, BUFFER_NUM, chunkFp16);
            pipe->InitBuffer(rFp32Buf, chunkFp16 * 2u);       // FP32: 2 bytes per half->float
            pipe->InitBuffer(sqBuf, chunkFp16 * 2u);          // FP32 chunk squares
            pipe->InitBuffer(sumSqBuf, static_cast<uint32_t>(this->SumAligned()) * sizeof(float));
...
private:
    // ------------------------------------------------------------------
    //  Aligned batch path (H % 16 == 0, H <= 4096): chunk-level copies and
    //  syncs, per-row FP32 compute, R kept in UB.
    // ------------------------------------------------------------------
    __aicore__ inline void ProcessAlignedBatch() {
        ...
        // Weight loaded once per core (full row), cast to FP32, reused.
        ...
        while (rowBase < myRows) {
            ...
            AscendC::LocalTensor<half> inLocal = inQue.AllocTensor<half>();
            ...
            inLocal = inQue.DeQue<half>();

            // ---- Phase A (chunk-wide vector ops, then per-row reduce) ----
            // R2: rows are contiguous in UB, so the per-row elementwise chain
            // (Cast/Cast/Add/Cast/Mul) becomes ONE op over n*alignH elements.
            ...
            AscendC::Cast(sq, inLocal[static_cast<uint32_t>(n) * static_cast<uint32_t>(alignH)],
                          AscendC::RoundMode::CAST_NONE, chunkElems);
            AscendC::Add(rFp32, rFp32, sq, chunkElems);
            ...
            // ---- rstd on the scalar unit: 1/sqrt(sum*invH + eps), ONE V->S
            // sync for the whole chunk ----
            AscendC::SetFlag<AscendC::HardEvent::V_S>(EVENT_ID0);
            AscendC::WaitFlag<AscendC::HardEvent::V_S>(EVENT_ID0);

            // ---- Phase B: per-row Muls(rstd) + Mul(weight), then ONE cast ----
...
private:
    ...
    // Batch path buffers.
    AscendC::TQue<AscendC::TPosition::VECIN, BUFFER_NUM> inQue;
