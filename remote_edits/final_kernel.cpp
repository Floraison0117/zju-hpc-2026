...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR residual, GM_ADDR weight,
        ...
        if (this->useBatch) {
            ...
            pipe->InitBuffer(inQue, BUFFER_NUM, 2u * chunkFp16);
...
private:
    ...
    __aicore__ inline void ProcessAlignedBatch() {
        ...
        while (rowBase < myRows) {
            ...
            // ---- rstd = 1 / sqrt(mean + eps) for all rows ----------------
            // Each row's sum lives at sumSqArr[row*8] (32B-aligned FP32 lane).
            // Do the tiny vector math per lane: Muls(invH), Adds(eps), Sqrt.
            // Scalar rstd experiment: read sumSq straight off the reduce result.
            // mean+eps and sqrt are done on the AIV scalar unit (sqrtf must
            // lower to a scalar instruction or be rejected by the compiler).
            AscendC::SetFlag<AscendC::HardEvent::V_S>(EVENT_ID0);
            AscendC::WaitFlag<AscendC::HardEvent::V_S>(EVENT_ID0);

            // ---- Phase B: y = R * rstd * weight (FP32), cast FP16 ----
