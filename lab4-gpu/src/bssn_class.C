...
void bssn_class::AnalysisStuff(int lev, double dT_lev)
{
  ...
  // black hole's position: output every step for check.sh trajectory matching
  {
    double *pox;
...
void bssn_class::Constraint_Out()
{
  ...
  // Constraint violation
  {
    ...
    for (int lev = 0; lev < GH->levels; lev++)
    {
      // level 0 constraints are reused from RK4 rhs; skip lev > 0 recompute
      if (false) // if the constrait quantities can be reused from the step rhs calculation
      {
        double TRK4 = PhysTime;
