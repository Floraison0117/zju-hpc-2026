                         ...
                         BLASINT lda, int kc, int m)
{
    ...
    // This halves pack traffic when kc == KC/2 (the K=128->64 sub-problems of
    // the recursion), where pack cost otherwise approaches compute cost.
}
...
static void strassen1(const zdouble* Ap, const zdouble* Bp, zdouble* Cp,
                      BLASINT M, BLASINT N, BLASINT K,
                      ...
                      BLASINT lda, BLASINT ldb, BLASINT ldc)
{
    ...
    // P1 = (A11+A22) @ (B11+B22)     [mh x nh, K=kh]
    {
        ...
        if (S && T) {
            ...
            gemm_core(S, T, P1, mh, nh, kh, 1.0, 0.0, kh, nh, nh);
        }
        free(S); free(T);
    }
    ...
    // P2 = (A21+A22) @ B11          [m2 x nh, K=kh]
    {
        ...
        if (S) {
            ...
            gemm_core(S, B11, P2, m2, nh, kh, 1.0, 0.0, kh, nh, nh);
        }
        free(S);
    }
    ...
    // P3 = A11 @ (B12 - B22)        [mh x n2, K=kh]
    {
        ...
        if (T) {
            ...
            gemm_core(A11, T, P3, mh, n2, kh, 1.0, 0.0, kh, n2, n2);
        }
        free(T);
    }
    ...
    // P4 = A22 @ (B21 - B11)        [m2 x nh, K=k2]
    {
        ...
        if (T) {
            ...
            gemm_core(A22, T, P4, m2, nh, k2, 1.0, 0.0, k2, nh, nh);
        }
        free(T);
    }
    ...
    // P5 = (A11 + A12) @ B22        [mh x n2, K=k2]
    {
        ...
        if (S) {
            ...
            gemm_core(S, B22, P5, mh, n2, k2, 1.0, 0.0, k2, n2, n2);
        }
        free(S);
    }
    ...
    // P6 = (A21 - A11) @ (B11 + B12)   [m2 x nh, K=kh]
    {
        ...
        if (S && T) {
            ...
            gemm_core(S, T, P6, m2, nh, kh, 1.0, 0.0, kh, nh, nh);
        }
        free(S); free(T);
    }
    ...
    // P7 = (A12 - A22) @ (B21 + B22)   [mh x nh, K=k2]
    {
        ...
        if (S && T) {
            ...
            gemm_core(S, T, P7, mh, nh, k2, 1.0, 0.0, k2, nh, nh);
        }
        free(S); free(T);
    }
                 ...
                 const BLASINT ldc)
{
    ...
    if (Order == CblasRowMajor && TransA == CblasNoTrans && TransB == CblasNoTrans) {
        ...
        if (M >= STRASSEN_MIN_M && N >= STRASSEN_MIN_N && K >= STRASSEN_MIN_K)
            strassen1(Ap, Bp, Cp, M, N, K, alpha_val, beta_val, lda, ldb, ldc);
        else
